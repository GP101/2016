...
int IsHorizontalFive( BOARD board, int row, int col )
{
    ...
    const int colEnd = col + 4;
    int stoneCounter = 1;
    for( int c = col + 1; c <= colEnd; ++c )
    {
        const int stone = GetStone( board, row, c );
        if( stone == firstStone )
        {
            stoneCounter += 1;
        }//if
...
{
    const int firstStone = GetStone( board, row, col );
    const int rowEnd = row + 4;
    int stoneCounter = 1;
    for( int r = row + 1; r <= rowEnd; ++r )
    {
        const int stone = GetStone( board, r, col );
        if( stone == firstStone )
        {
            stoneCounter += 1;
        }//if
